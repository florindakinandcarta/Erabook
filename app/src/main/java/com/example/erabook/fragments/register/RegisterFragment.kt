...
class RegisterFragment : Fragment() {
    ...
    binding.apply {
      emailInput.editText?.addTextChangedListener { editable ->
        if (editable.toString().trim().isNotEmpty() &&
            !android.util.Patterns.EMAIL_ADDRESS.matcher(editable.toString().trim()).matches()) {
      ...
      confirmPasswordInput.editText?.addTextChangedListener { passwordEditable ->
        if (passwordInput.editText?.text.toString().trim() != passwordEditable.toString().trim()) {
  ...
  private fun setupOnClickListeners() {
    binding.apply {
      ...
      backRegister.setOnClickListener { requireActivity().onBackPressedDispatcher.onBackPressed() }
    }
  }
}
