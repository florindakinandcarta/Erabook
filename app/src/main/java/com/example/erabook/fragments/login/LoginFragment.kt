...
class LoginFragment : Fragment() {
      ...
      authenticationViewModel.firebaseAuthWithGoogle(
          account?.idToken,
          {
            UserDataRemote(
                ...
                .let { userInfoViewModel.addUserData(it) }
  ...
  private fun setupOnClickListeners() {
    binding.apply {
      goToRegister.setOnClickListener { findNavController().navigate(R.id.loginToRegister) }
      ...
      googleButton.setOnClickListener {
        ...
        signInIntent?.let { intent -> startSignInActivityForResult.launch(intent) }
      }
            ...
            if (task == true) {
              ...
              authenticationViewModel.exception.observe(viewLifecycleOwner) { message ->
                when (message) {
                  ...
                  getString(R.string.error_code_message_incorrect) ->
                      requireContext().showToast(R.string.wrong_password_email)
                  getString(R.string.error_code_message_connection) ->
                      requireContext().showToast(R.string.error_connection)
                }
              }
            }
          }
